...
class PDFExtractor:
    ...
    def extract_text(self, pdf_path: str) -> Dict[str, any]:
        ...
        if len(combined) < 60:
            logger.info(" Low-text PDF detected; using OCR rasterization fallback")
            ...
            try:
                ...
                if result and result[0]:
                    ...
                    if page_text:
                        ...
                        logger.info(f"✓ OCR page {i}: {len(lines)} lines")
                    else:
                        logger.warning(f" OCR page {i}: no confident lines")
                else:
                    logger.warning(f"OCR page {i}: empty result")
            except Exception as e:
